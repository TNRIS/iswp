<script>
    ...
    let showDecade = async (event) => {
        ...
        if ($bindTreeMap) $bindTreeMap(data);

        if ($buildPie) $buildPie();
    };
...
<div
    ...
    {#if !hideTheme}
        <div>
            ...
            <ThemeSelector {showPopulation} show={showTheme} bind:select_theme={$themeStore} {sourcePage} id_pre="plan_dec_theme" />
        </div>
    {/if}
</div>
